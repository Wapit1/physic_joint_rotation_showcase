...
#	target_forward = target_forward.rotated(Vector3(0,0,1),-target_rot.z)
	...
	var torque : Vector3 = target_forward.z.cross(-transform.basis.z)
	apply_torque_impulse(torque/10)
	apply_torque_impulse(target_forward.y.cross(-transform.basis.y)/10)
	apply_torque_impulse(target_forward.x.cross(-transform.basis.x)/10)
